...
(defun space->underscore (string)
  (if string
      (if (listp string)
          (if (char= (car string) #\SPACE)
              (progn
                (cons #\_ (space->underscore (cdr string))))
              (progn
                (cons (car string) (space->underscore (cdr string)))))
          (coerce (space->underscore (coerce string 'list)) 'string))
      nil))

;; creating of the tk widget after creating the clos object
(defmethod initialize-instance :after ((w widget) &key)
  (if (name w)
      (when (not (string= (string-downcase (space->underscore (name w))) (name w)))
        (error (format nil "the chosen name of '~a' either contains illegal whitespace or capital letters" (name w))))
